...
typewriter.pauseFor(500).typeString('Hi, my name is<br><span style="color: #f5c708;">Maciej Ked</span>').pauseFor(100).deleteChars(3).typeString('<span style="color: #f5c708;">Kędziora</span><br>I program websites &<br>microcontrollers').start();
/*fade out video on scroll*/

var video = document.querySelector('video');
window.addEventListener('scroll', function () {
  var value = 1 + window.scrollY / -400;
  ...
  function divideLetters() {
    data.target.forEach(function (element, index) {
      ...
      for (var i = 0; i < text.length; i++) {
        textDivided += "<span class=\"el-sp el-st-".concat(index, "-span-").concat(i, "\" data-txt=\"").concat(text.substr(i, 1), "\">").concat(text.substr(i, 1), "</span>");
      }
  ...
  return {
    init: function init(selector) {
      ...
      if (checkLength(selector)) {
        // salvo le stringhe
        data.target = Array.from(document.querySelectorAll(selector)); // divido le singole stringhe in lettere

        divideLetters(); // lancio la funzione che si ripete

        changeLetters(); // aggiungo il listener allo scroll
